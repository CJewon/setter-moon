...
export const productEditVariantSchema = z.object({
  id: z.string().uuid("옵션 조합을 다시 선택해 주세요."),
  isActive: z.boolean()
});
...
export const productEditSchema = z.object({
  ...
  variants: z.array(productEditVariantSchema).min(1, "옵션 조합을 1개 이상 확인해 주세요.").optional()
}).superRefine((value, context) => {
  if (!value.variants?.length || value.status === "hidden") {
    return;
  }

  const activeVariantCount = value.variants.filter((variant) => variant.isActive).length;

  if (activeVariantCount === 0) {
    context.addIssue({
      code: "custom",
      message: "판매중 또는 품절 상품은 사용할 옵션 조합이 1개 이상 필요합니다.",
      path: ["variants"]
    });
  }
});
...
export type ProductEditVariantValues = z.infer<typeof productEditVariantSchema>;
