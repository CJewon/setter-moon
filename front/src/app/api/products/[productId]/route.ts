...
export const GET = withApiErrorBoundary(async (_request: Request, { params }: ProductRouteContext) => {
  if (!hasSupabasePublicEnv()) {
    return errorResponse(500, "상품 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
  }
  ...
  if (!access.store) {
    return errorResponse(404, "스토어를 먼저 만들어야 상품 정보를 볼 수 있습니다.");
  }

  try {
    return successResponse(await getProductDetailForStore(supabase, access.store.id, productId), {
      message: "상품 정보를 불러왔습니다."
    });
  } catch (error) {
