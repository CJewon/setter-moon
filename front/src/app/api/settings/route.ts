...
export const GET = withApiErrorBoundary(async () => {
  if (!hasSupabasePublicEnv()) {
    return errorResponse(500, "설정 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
  }
...
export const PATCH = withApiErrorBoundary(async (request: Request) => {
  if (!hasSupabasePublicEnv()) {
    return errorResponse(500, "설정 정보를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요.");
  }

  const parsed = await parseJsonBody(request, settingsFormSchema, "입력한 설정 정보를 다시 확인해 주세요.");
  ...
  try {
    ...
    if (isStoreMutationError(error) && (error.code === "42501" || error.code === "PGRST301")) {
      return errorResponse(403, "설정을 수정할 권한을 확인하지 못했습니다. 다시 로그인해 주세요.");
    }
